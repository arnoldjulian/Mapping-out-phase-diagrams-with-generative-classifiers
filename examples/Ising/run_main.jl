...
n_classes = 5
class_data = zeros(eltype(n_classes),length(γ1_range),length(γ2_range),n_classes)
for class_indx in 1:n_classes
	if class_indx == 1
        class_data[10,10,class_indx] = 1
	elseif class_indx == 2
        class_data[10,length(γ2_range)-10,class_indx] = 1
	elseif class_indx == 3
        class_data[length(γ1_range)-10,10,class_indx] = 1
	elseif class_indx == 4
        class_data[length(γ1_range)-10,length(γ2_range)-10,class_indx] = 1
	else
		class_data[Int(length(γ1_range)/2),Int(length(γ2_range)/2),class_indx] = 1
	end
end

# obtain indicator of scheme 1 based on set of generative models
I_1 = GCPT.run_scheme_1(x_data, class_data, [dγ1, dγ2])
...
plot!(ref_phase_boundary[:,1,1],ref_phase_boundary[:,2,1],color="green",label="analytical")
plot!(ref_phase_boundary[:,1,2],ref_phase_boundary[:,2,2],color="green",label=false)
plot!(ref_phase_boundary[:,1,3],ref_phase_boundary[:,2,3],color="green",label=false)
plot!(ref_phase_boundary[:,1,4],ref_phase_boundary[:,2,4],color="green",label=false)
...
I_2 = GCPT.run_scheme_2(x_data,l_param,γ1_range,γ1_range_LBC,γ2_range,γ2_range_LBC,[dγ1,dγ2])
...
plt = contourf(x, y, (x,y)->f(x,y,I_2), fill=true,dpi=300,color=:thermal,levels=1000)
xlabel!(L"J_{x}/k_{\mathrm{B}}T}")
ylabel!(L"J_{y}/k_{\mathrm{B}}T}")
plot!(ref_phase_boundary[:,1,1],ref_phase_boundary[:,2,1],color="green",label="analytical")
plot!(ref_phase_boundary[:,1,2],ref_phase_boundary[:,2,2],color="green",label=false)
...
plt = contourf(x, y, (x,y)->f(x,y,I_3), fill=true,dpi=300,color=:thermal,levels=1000)
xlabel!(L"J_{x}/k_{\mathrm{B}}T}")
ylabel!(L"J_{y}/k_{\mathrm{B}}T}")
plot!(ref_phase_boundary[:,1,1],ref_phase_boundary[:,2,1],color="green",label="analytical")
plot!(ref_phase_boundary[:,1,2],ref_phase_boundary[:,2,2],color="green",label=false)
