...
scatter!(inv_rescale_x.(snake.vertices[:,1]),inv_rescale_y.(snake.vertices[:,2]),label="Snake: Epoch 0",color="black")
...
for epoch in 1:n_epochs
    ...
    if epoch % 50 == 0
        println(epoch)
        scatter!(inv_rescale_x.(snake.vertices[:,1]),inv_rescale_y.(snake.vertices[:,2]),label="Snake: Epoch "*string(epoch),color=colors[epoch])
        plot!(inv_rescale_x.(snake.vertices[:,1]),inv_rescale_y.(snake.vertices[:,2]),label=false,color=colors[epoch])
    end
end
