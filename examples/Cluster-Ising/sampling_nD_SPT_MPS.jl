...
prob = vcat(readdlm(data_folder*"/probs_Pauli6_h2="*string(0)*".txt",eltype(Float64))...)
...
for j in 1:length(p2_range)
	x_data[j,:] = vcat(readdlm(data_folder*"/probs_Pauli6_h2="*string(j-1)*".txt",eltype(Float64))...) 
	p_data[j,:] = [p2_range[j]]
end
...
for j in 1:length(p2_range)
	for i in 1:length(p1_range)
		wavefunc_data[i,j,:] .= vcat(readdlm(data_folder*"/gs_real_h1="*string(i-1)*"_h2="*string(j-1)*".txt",eltype(Float32))...) .+ vcat(readdlm(data_folder*"/gs_imag_h1="*string(i-1)*"_h2="*string(j-1)*".txt",eltype(Float32))...)*im 
	end
end
...
pred_SL_opt, indicator_SL_opt_classes, indicator_SL_opt_global = MLP.get_indicators_SL_analytical_nD(abs2.(wavefunc_data[17,:,:]), class_data, [dp2])
pred_SL_opt_est = MLP.get_mean_obs_SL_nD_f(1000, class_data_prob, [dp2], abs2.(wavefunc_data[17,:,:]))
pred_SL_opt_est_MPS = MLP.get_mean_obs_SL_nD_f_MPS(1000,class_data_prob,[dp2],MPS_data,sites,L)
...
pred_PBM_opt, _, divergence_opt, loss_opt = MLP.get_indicators_PBM_analytical_nD(abs2.(wavefunc_data[17,:,:]), p_data, [dp2])
pred_PBM_opt_est = MLP.get_mean_obs_PBM_nD_f(1000,p_data,[dp2],abs2.(wavefunc_data[17,:,:]))
...
ind_LBC_opt, _ = MLP.get_indicators_LBC_analytical(abs2.(wavefunc_data[17,:,:])',p2_range)
ind_LBC_opt_est = MLP.get_mean_obs_LBC_1D_f_old_ind(100,p2_range,abs2.(wavefunc_data[17,:,:]))
ind_LBC_opt_est_MPS = MLP.get_mean_obs_LBC_1D_f_old_ind_MPS(100,p2_range,MPS_data,sites,L)
...
_, ind_LBC_opt_TV = MLP.get_indicators_LBC_analytical_unbiased_range(abs2.(wavefunc_data[17,:,:])', p2_range, n_neighbors)
ind_LBC_opt_TV_est = map(p_tar_indx->MLP.get_mean_obs_LBC_1D_f_TV(100,p2_range,abs2.(wavefunc_data[17,:,:]),n_neighbors,p_tar_indx),1:length(p2_range)-1)
ind_LBC_opt_TV_est_MPS = map(p_tar_indx->MLP.get_mean_obs_LBC_1D_f_TV_MPS(100,p2_range,MPS_data,sites,L,n_neighbors,p_tar_indx),1:length(p2_range)-1)
