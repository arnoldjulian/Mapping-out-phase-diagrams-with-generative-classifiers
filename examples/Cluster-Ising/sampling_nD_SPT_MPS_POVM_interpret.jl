...
L = 7
data_folder = "/media/julian/Seagate Expansion Drive/2D_scans/SPT_POVM/data/L=" *
              string(L) * "/run=1/"
...
function interpret_wf(p, wavefunc_data, L, wavefunc_int)
    ...
    sampl = StatsBase.sample(1:length(@view wavefunc_data[1, :]),
        Weights(abs2.(unit * (@view wavefunc_data[p[1], :]))),
        1)[1]
    return seq .* wavefunc_int[sampl, :],
    (abs2.(unit * (@view wavefunc_data[p[1], :])))[sampl] / (3^L),
    sampl,
    unit
end
...
for n in 1:n_samples
    ...
    #for p_indx in 1:15
    for p_indx in 20:80
        seq, prob, sampl, unit = interpret_wf([p_indx],
            wavefunc_data[17, :, :],
            L,
            wavefunc_int)
        prob_SL = MLP.get_obs_SL_nD_f_wf((sampl, unit),
            class_data_prob,
            [dp2],
            wavefunc_data[17, :, :],
            L)[2]
...
obs_list = map(p -> real(adjoint(wavefunc_data[17, p, :]) * obs * wavefunc_data[17, p, :]),
    1:length(p2_range))
...
pred_SL_opt, indicator_SL_opt_classes, indicator_SL_opt_global = MLP.get_indicators_SL_analytical_nD(x_data,
    class_data,
    [dp2])
pred_SL_opt_est = MLP.get_mean_obs_SL_nD_f_wf(100,
    class_data_prob,
    [dp2],
    wavefunc_data[17, :, :],
    L)
pred_SL_opt_est_MPS = MLP.get_mean_obs_SL_nD_f_MPS(100,
    class_data_prob,
    [dp2],
    MPS_data,
    sites,
...
ind_LBC_opt_est = MLP.get_mean_obs_LBC_1D_f_old_ind_wf(1,
    p2_range,
    wavefunc_data[17, :, :],
    L)
...
_, ind_LBC_opt_TV = MLP.get_indicators_LBC_analytical_unbiased_range(x_data',
    p2_range,
    n_neighbors)
ind_LBC_opt_TV_est = map(p_tar_indx -> MLP.get_mean_obs_LBC_1D_f_TV_wf(10,
        p2_range,
        wavefunc_data[17, :, :],
        L,
        n_neighbors,
        p_tar_indx),
    1:(length(p1_range) - 1))
ind_LBC_opt_TV_est_MPS = map(p_tar_indx -> MLP.get_mean_obs_LBC_1D_f_TV_MPS(10,
        p2_range,
        MPS_data,
        sites,
        L,
