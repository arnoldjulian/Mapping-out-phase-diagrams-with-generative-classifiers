...
pred_SL_opt, indicator_SL_opt_classes, indicator_SL_opt_global = MLP.get_indicators_SL_analytical_nD(x_data,
    class_data,
    [dp2])
pred_SL_opt_est = MLP.get_mean_obs_SL_nD_f_wf(1000,
    class_data_prob,
    [dp2],
    wavefunc_data[17, :, :],
    L)

plot(p2_range, pred_SL_opt[:, 1], color = "black", label = false)
plot!(p2_range, pred_SL_opt[:, 2], color = "black", label = false)
plot!(p2_range, pred_SL_opt[:, 3], color = "black", label = false)

scatter!(p2_range, pred_SL_opt_est[:, 1, 1], color = "red", label = false)
scatter!(p2_range, pred_SL_opt_est[:, 1, 2], color = "blue", label = false)
scatter!(p2_range, pred_SL_opt_est[:, 1, 3], color = "green", label = false)

pred_PBM_opt, _, divergence_opt, loss_opt = MLP.get_indicators_PBM_analytical_nD(x_data,
    p_data,
    [dp2])
pred_PBM_opt_est = MLP.get_mean_obs_PBM_nD_f_wf(1000,
    p_data,
    [dp2],
    wavefunc_data[17, :, :],
    L)

plot(p2_range[2:(end - 1)], divergence_opt)
plot(p2_range, pred_PBM_opt)
scatter!(p2_range, pred_PBM_opt_est)

ind_LBC_opt, _ = MLP.get_indicators_LBC_analytical(x_data', p2_range)
ind_LBC_opt_est = MLP.get_mean_obs_LBC_1D_f_old_ind_wf(100,
    p2_range,
    wavefunc_data[17, :, :],
    L)

plot(p1_range_LBC, ind_LBC_opt)
plot!(p1_range_LBC, ind_LBC_opt_est)

n_neighbors = 4
_, ind_LBC_opt_TV = MLP.get_indicators_LBC_analytical_unbiased_range(x_data',
    p2_range,
    n_neighbors)
ind_LBC_opt_TV_est = map(p_tar_indx -> MLP.get_mean_obs_LBC_1D_f_TV_wf(1000,
        p2_range,
        wavefunc_data[17, :, :],
        L,
        n_neighbors,
...
plot(p1_range_LBC[2:(end - 1)], ind_LBC_opt_TV)
plot!(p1_range_LBC[2:(end - 1)], ind_LBC_opt_TV_est)

@btime MLP.get_mean_obs_SL_nD_f_wf(10, class_data_prob, [dp2], wavefunc_data[17, :, :], L)
