...
function initialize_snake(vertices,
    boundaries,
    p1_range,
    p2_range;
    PBC = false,
    ...
    train_local = false,
    max_move = false,
    max_moves = [1.0f0, 1.0f0],
    treat_boundary = false)
    snake = Snake(vertices,
        size(vertices)[1],
        PBC,
        FBC,
        zeros(eltype(w0), size(vertices)[1], n_nodes, size(vertices)[2]),
        n_nodes,
        w0 * ones(eltype(w0), size(vertices)[1]),
        zeros(eltype(vertices[1, 1]), size(vertices)),
        zeros(eltype(w0), size(vertices)),
        α,
        β,
        γ,
        w0,
    ...
    for i in 1:(snake.n_vertices)
        for j in 1:(snake.n_nodes)
            if j <= snake.n_nodes / 2
                snake.nodes[i, j, :] = snake.vertices[i, :] .+
                                       (Int((snake.n_nodes / 2 - j) + 1) *
                                        snake.widths[i] .* snake.normals[i, :])
            else
                snake.nodes[i, j, :] = snake.vertices[i, :] .+
                                       ((-1 * (j - Int(snake.n_nodes / 2)) *
                                         snake.widths[i]) .* snake.normals[i, :])
            end
        end
    end
...
function get_loss_LBC_fast_2D_snake(data,
    n_samples,
    nD_p_space,
    oneD_p_space,
    p_g,
    ...
    p1_range,
    p2_range)
    data_matrix = hcat(map(p -> bilinear_interpol(data,
            p1_range,
            p2_range,
            nD_p_space[p, :]),
        1:size(nD_p_space)[1])...)
    guesses = reshape(map(p -> GCPT.guesser(p, p_g, σ), oneD_p_space),
        (1, size(data_matrix)[2]))

    p1 = sum(data_matrix .* guesses, dims = 2)[:, 1]
    p2 = sum(data_matrix .* (ones(eltype(guesses[1, 1]), size(guesses)) .- guesses),
        dims = 2)[:,
        1]
    pred_opt = [if p1[i] + p2[i] > eps(eltype(guesses[1, 1]))
        p1[i] / (p1[i] + p2[i])
    else
        zero(eltype(guesses[1, 1]))
    end
    ...
    return sum(map(p_indx -> ((crossentropy.(pred_opt, guesses[p_indx])') *
                              (@view data_matrix[:, p_indx])),
        1:length(guesses))) / length(guesses)
end
...
function get_β_term(snake::Snake, vertices)
    ...
    for i in set
        if i == 1
            β_term += norm((vertices[3, :] .- (2 * vertices[2, :]) .+ vertices[1, :]) ./
                           (seg_length)^2)^2
        elseif i == snake.n_vertices
            β_term += norm((vertices[i, :] .- (2 * vertices[i - 1, :]) .+
                            vertices[i - 2, :]) ./ (seg_length)^2)^2
        else
            β_term += norm((vertices[i + 1, :] .+ vertices[i - 1, :] .-
                            (2 * vertices[i, :])) ./ (seg_length)^2)^2
        end
    end
...
function get_grad(snake::Snake, data, n_samples)
    ...
    Threads.@threads for i in 1:(snake.n_vertices)
        oneD_p_space = (collect(1:(snake.n_nodes + 1)) .- Int(snake.n_nodes / 2 + 1)) .*
                       snake.widths[i]
        deleteat!(oneD_p_space, Int(snake.n_nodes / 2 + 1))

        if snake.train_local
            loss = p -> get_loss_LBC_fast_2D_snake(data,
                n_samples,
                clip_nodes(snake, snake.nodes[i, :, :]),
                oneD_p_space,
                p[1],
                snake.widths[i],
                snake.p1_range,
            ...
            loss = p -> get_loss_LBC_fast_2D_snake(data,
                ...
                clip_nodes(snake,
                    snake.nodes[i, :, :] .-
                    p[1] .* repeat(snake.normals[i, :]', snake.n_nodes, 1)),
                oneD_p_space .+ p[1],
                p[1],
                snake.widths[i] / 10,
                snake.p1_range,
...
function update_snake!(snake::Snake,
    data,
    n_samples,
    opt_internal_losses,
    opt_external_losses,
    ...
    if snake.FBC
        snake.vertices_update[1, :] = zeros(eltype(snake.vertices_update[1, 1]),
            size(snake.vertices_update[1, :]))
        snake.vertices_update[snake.n_vertices, :] = zeros(eltype(snake.vertices_update[1,
                1]),
            size(snake.vertices_update[1, :]))
    end

    if snake.max_move
        snake.vertices[:, 1] .+= snake.max_moves[1] .* (tanh.(snake.vertices_update[:, 1] .-
                                        snake.vertices[:, 1]))
        snake.vertices[:, 2] .+= snake.max_moves[2] .* (tanh.(snake.vertices_update[:, 2] .-
                                        snake.vertices[:, 2]))
    else
    ...
    if snake.reshape
        ...
        bckwrd = reverse(reshape_snake(snake, reverse(deepcopy(snake.vertices), dims = 1)),
            dims = 1)
        snake.vertices .= (frwrd .+ bckwrd) ./ 2
    end

    clip_snake!(snake)
    snake.widths .= width_sched(snake, epoch) *
                    ones(eltype(snake.widths[1]), size(snake.widths))
    set_nodes!(snake)
...
function bilinear_interpol(data, p1_range, p2_range, pnt)
    ...
    return w11 .* (@view data[arg_x_1, arg_y_1, :]) .+
           w12 .* (@view data[arg_x_1, arg_y_2, :]) .+
           w21 .* (@view data[arg_x_2, arg_y_1, :]) .+
           w22 .* (@view data[arg_x_2, arg_y_2, :])
end
