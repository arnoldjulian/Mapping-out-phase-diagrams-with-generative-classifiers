...
function get_sample(p, x_data)
    if length(p) == 1
        return StatsBase.sample(1:length(@view x_data[p[1], :]),
            Weights(@view x_data[p[1], :]),
            1)
    else
        return StatsBase.sample(1:length(@view x_data[p[1], p[2], :]),
            Weights(@view x_data[p[1], p[2], :]),
            1)
    end
end
