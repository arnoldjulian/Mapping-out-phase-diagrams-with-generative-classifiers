...
function get_sample_wf!(p, wavefunc_data, L, sampl)
    ...
    if length(p) == 1
        sampl[1] .= StatsBase.sample(1:length(@view wavefunc_data[1, :]),
            Weights(abs2.(sampl[2] * (@view wavefunc_data[p[1], :]))),
            1)[1]
        return sampl
    else
        sampl[1] .= StatsBase.sample(1:length(@view wavefunc_data[1, 1, :]),
            Weights(abs2.(sampl[2] * (@view wavefunc_data[p[1], p[2], :]))),
            1)[1]
        return sampl
    end
end
...
function get_mean_obs_SL_nD_f_wf(n_samples, class_data, dp, wavefunc_data, L)
    ...
    if param_space_dim == 1
        ...
        for j in 1:size(wavefunc_data)[1]
            for i in 1:n_samples
                mean_obs[j, 1, :] .+= get_obs_SL_nD_f_wf(get_sample_wf!([j],
                        wavefunc_data,
                        L,
                        sampl),
                    class_data,
    ...
    elseif param_space_dim == 2
        ...
        for k in 1:size(wavefunc_data)[2]
            for j in 1:size(wavefunc_data)[1]
                for i in 1:n_samples
                    mean_obs[j, k, :] .+= get_obs_SL_nD_f_wf(get_sample_wf!([j, k],
                            wavefunc_data,
                            L,
                            sampl),
                        class_data,
...
function get_mean_obs_LBC_1D_f_TV_wf(n_samples,
    p_range,
    wavefunc_data,
    L,
    n_neighbors,
    ...
    for j in range_I
        for i in 1:n_samples
            mean_obs += get_obs_LBC_1D_f_TV_wf(get_sample_wf!([j], wavefunc_data, L, sampl),
                p_range,
                wavefunc_data,
                L,
                range_I,
...
function get_mean_obs_LBC_1D_f_old_ind_wf(n_samples, p_range, wavefunc_data, L)
    ...
    for j in 1:length(p_range)
        for i in 1:n_samples
            mean_obs .+= get_obs_LBC_1D_f_old_ind_wf(get_sample_wf!([j],
                    wavefunc_data,
                    L,
                    sampl),
                p_range,
...
function get_mean_obs_PBM_nD_f_wf(n_samples, p_data, dp, wavefunc_data, L)
    ...
    if param_space_dim == 1
        ...
        for j in 1:size(p_data)[1]
            for i in 1:n_samples
                mean_obs[j, :] .+= get_obs_PBM_nD_f_wf(get_sample_wf!([j],
                        wavefunc_data,
                        L,
                        sampl),
                    p_data,
    ...
    elseif param_space_dim == 2
        ...
        for k in 1:size(p_data)[2]
            for j in 1:size(p_data)[1]
                for i in 1:n_samples
                    mean_obs[j, k, :] .+= get_obs_PBM_nD_f_wf(get_sample_wf!([j, k],
                            wavefunc_data,
                            L,
                            sampl),
                        p_data,
