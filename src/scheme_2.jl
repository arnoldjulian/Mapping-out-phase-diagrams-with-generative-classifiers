...
function run_scheme_2(x_data, l_param, γ1_range, γ1_range_LBC, γ2_range, γ2_range_LBC, dγ,n_samples)
    I_2_x = zeros(eltype(dγ[1]), length(γ1_range), length(γ2_range_LBC) - 2)
    for i in 1:length(γ1_range)
...
function run_scheme_2_loc(x_data, l_param, p_range, n_samples)
    I_2_loc = zeros(eltype(p_range[1]), length(p_range) - 1)

    for p_tar_indx in 1:(length(p_range) - 1)
        range_I, range_II = get_scheme_2_ranges(p_range, Int(l_param), p_tar_indx)
        I_2_loc[p_tar_indx] = run_scheme_2_fixed_bipartition(x_data,
            p_range,
            range_I,
            range_II, n_samples)
    end
...
function run_scheme_2_fixed_bipartition(x_data, p_range, range_I, range_II, n_samples)
	mean_error = zero(eltype(p_range[1]))

	for j in range_I
		for i in 1:n_samples
			mean_error += get_error_scheme_2(x_data,p_range,range_I,range_II,get_sample([j],x_data))/(2*length(range_I))
		end
	end

    for j in range_II
		for i in 1:n_samples
			mean_error += get_error_scheme_2(x_data,p_range,range_I, range_II,get_sample([j],x_data))/(2*length(range_II))
		end
	end
...
function get_error_scheme_2(x_data,p_range,range_I,range_II,sampl)
