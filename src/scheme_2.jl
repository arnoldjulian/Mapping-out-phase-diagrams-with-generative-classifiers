...
function run_scheme_2_fixed_bipartition(data, p_range, range_I, range_II)
  p1 = sum((@view data[:,range_I]),dims=2,init=zero(eltype(p_range[1])))[:,1]./length(range_I)
  p2 = sum((@view data[:,range_II]),dims=2,init=zero(eltype(p_range[1])))[:,1]./length(range_II)
  pred_opt = p1./(p1.+p2.+eps(eltype(p_range[1])))

  prob_eff = ((sum((@view data[:,range_I]),dims=2)./length(range_I)).+(sum((@view data[:,range_II]),dims=2)./length(range_II)))./2
  error = sum(prob_eff.*(min.(pred_opt,ones(eltype(p_range[1]),length(pred_opt)).-pred_opt)))
...
function run_scheme_2_loc(data, p_range, l_param)
  ...
  for p_tar_indx in 1:length(p_range)-1
    ...
    I_2_loc[p_tar_indx] = run_scheme_2_fixed_bipartition(data, p_range, range_I, range_II)
  end
