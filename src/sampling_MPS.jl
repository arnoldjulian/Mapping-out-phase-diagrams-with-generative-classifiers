    ...
    if param_space_dim == 1
        ...
        for j in 1:size(MPS_data)[1]
            for i in 1:n_samples
                mean_obs[j,1,:] .+= get_obs_SL_nD_f_MPS(get_sample_MPS([j],MPS_data,sites,L),class_data,dp,MPS_data, sites, L)
            end
        end
      ...
      elseif param_space_dim == 2
        ...
        for k in 1:size(MPS_data)[2]
            for j in 1:size(MPS_data)[1]
                for i in 1:n_samples
                    mean_obs[j,k,:] .+= get_obs_SL_nD_f_MPS(get_sample_MPS([j,k],MPS_data,sites,L),class_data,dp,MPS_data, sites, L)
                end
            end
        end
...
function get_mean_obs_LBC_1D_f_TV_MPS(n_samples,p_range,MPS_data,sites,L,n_neighbors,p_tar_indx)
	...
	for j in range_I
		for i in 1:n_samples
			mean_obs += get_obs_LBC_1D_f_TV_MPS(get_sample_MPS([j],MPS_data,sites,L),p_range,MPS_data,sites,L,range_I, range_II)/(2*length(range_I))
		end
	end

    for j in range_II
		for i in 1:n_samples
			mean_obs += get_obs_LBC_1D_f_TV_MPS(get_sample_MPS([j],MPS_data,sites,L),p_range,MPS_data,sites,L,range_I, range_II)/(2*length(range_II))
		end
	end
...
function get_mean_obs_LBC_1D_f_old_ind_MPS(n_samples,p_range,MPS_data,sites,L)
	...
	for j in 1:length(p_range)
		for i in 1:n_samples
			mean_obs .+= get_obs_LBC_1D_f_old_ind_MPS(get_sample_MPS([j],MPS_data,sites,L),p_range,MPS_data,sites,L)
		end
	end
...
function get_mean_obs_PBM_nD_f_MPS(n_samples,p_data,dp,MPS_data,sites,L)
    ...
    if param_space_dim == 1
        for j in 1:size(p_data)[1]
            for i in 1:n_samples
                mean_obs[j,:] .+= get_obs_PBM_nD_f_MPS(get_sample_MPS([j],MPS_data,sites,L),p_data,dp,MPS_data,sites,L)
            end
        end
      ...
      elseif param_space_dim == 2
        for k in 1:size(p_data)[2]
            for j in 1:size(p_data)[1]
                for i in 1:n_samples
                    mean_obs[j,k,:] .+= get_obs_PBM_nD_f_MPS(get_sample_MPS([j,k],MPS_data,sites,L),p_data,dp,MPS_data,sites,L)
                end
            end
        end
