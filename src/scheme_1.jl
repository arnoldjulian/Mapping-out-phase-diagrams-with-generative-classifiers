function run_scheme_1(x_data, class_data, dp)
  ...
  for i in 1:n_classes-1
    ...
    pred_x[:,i] = [if pred_x_prov_sum[j] != zero(eltype(x_data_r[1,1])) pred_x_prov[j,i]/(pred_x_prov_sum[j]) else zero(eltype(x_data_r[1,1])) end for j in 1:size(x_data_r)[1]]
  end
  pred_x[:,n_classes] = ones(eltype(x_data_r[1,1]),size(x_data_r)[1]).-sum(pred_x,dims=2)
  ...
  if param_space_dim == 1
    ...
    for i in 1:n_classes
      grad = (circshift(@view(mean_pred[:,i]),(-1)).-circshift(@view(mean_pred[:,i]),(1)))./(2*dp[1])
      I_1_classes[:,i] = @view sqrt.(grad.^2)[2:end-1]
    end

  elseif param_space_dim == 2
    I_1_classes = zeros(eltype(x_data_r[1,1]),size(x_data)[1]-2,size(x_data)[2]-2,n_classes)

    for i in 1:n_classes
      grad_1 = (circshift(@view(mean_pred[:,:,i]),(-1,0)).-circshift(@view(mean_pred[:,:,i]),(1,0)))./(2*dp[1])
      grad_2 = (circshift(@view(mean_pred[:,:,i]),(0,-1)).-circshift(@view(mean_pred[:,:,i]),(0,1)))./(2*dp[2]) 
      I_1_classes[:,:,i] = @view sqrt.(grad_1.^2 .+ grad_2.^2)[2:end-1,2:end-1]
    end
  else
