...
function run_scheme_3(x_data,p_data,dp,n_samples)
    param_space_dim = length(dp)
	mean_pred = zeros(eltype(dp[1]),size(p_data))
    mean_pred_sq = zeros(eltype(dp[1]),size(p_data))

    if param_space_dim == 1
        for j in 1:size(p_data)[1]
            for i in 1:n_samples
                pred = get_pred_scheme_3(x_data,p_data,dp,get_sample([j],x_data))
                mean_pred[j,:] .+= pred
...
function get_pred_scheme_3(x_data,p_data,dp,sampl)
    param_space_dim = length(dp)
	pred = zeros(eltype(dp[1]),length(dp))
...
function run_scheme_3_linescans(x_data, p_data, dγ, n_samples)
    ...
    for i in 1:length(γ1_range)
        _, _, I_3_y_shot = run_scheme_3(x_data[i, :, :], p_data[i, :, 2], [dγ[2]], n_samples)
        I_3_y[i, :] .= I_3_y_shot
    end

    for i in 1:length(γ2_range)
        _, _, I_3_x_shot = run_scheme_3(x_data[:, i, :], p_data[:, i, 1], [dγ[1]], n_samples)
        I_3_x[:, i] .= I_3_x_shot
    end
