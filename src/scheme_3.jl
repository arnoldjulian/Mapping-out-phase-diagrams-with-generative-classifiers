function run_scheme_3(x_data, p_data, dp)
  ...
  for i in 1:param_space_dim
    pred[:,i] = sum(x_data_r.*(@view p_data_r[:,i])',dims=2)./(sum(x_data_r,dims=2).+eps(eltype(dp[i])))
  ...
  if param_space_dim == 1
    I_3  = (((circshift((@view mean_pred[:,1]),(-1)).-circshift((@view mean_pred[:,1]),(1)))./(2*dp[1]))[2:end-1])./std[2:end-1]
  elseif param_space_dim == 2
    sig_x = ((circshift((@view mean_pred[:,:,1]),(-1,0)).-circshift((@view mean_pred[:,:,1]),(1,0)))./(2*dp[1]))[2:end-1,2:end-1]
    sig_y = ((circshift((@view mean_pred[:,:,2]),(0,-1)).-circshift((@view mean_pred[:,:,2]),(0,1)))./(2*dp[2]))[2:end-1,2:end-1]
    I_3  = sqrt.(((sig_x./std[2:end-1,2:end-1,1]).^2) .+ ((sig_y./std[2:end-1,2:end-1,2]).^2))
  else
